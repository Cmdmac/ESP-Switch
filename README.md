...
> 📖 **终端用户**：配网、日常使用、定时、常见问题请看 **[用户使用说明（USER_GUIDE.md）](USER_GUIDE.md)**；本 README 面向开发者（编译、部署、硬件）。
...
- **C2 为什么走 IDF**：arduino-esp32 3.x 删掉了 `esp32c2-libs` 预编译包，arduino-cli/PIO 无法直接编 C2；把 arduino-esp32 作为 **ESP-IDF 组件**编译时，bootloader/SDK 由 IDF 5.x 生成（本项目已验证 5.5.2），绕开缺失的包。C2 板型因此**只出现在网页 Tab2**（Tab1 下拉不含 C2）。
...
1. 手机或电脑连接 Wi-Fi：`ESPSwitch-XXXX`（XXXX = 设备 MAC 后两字节，配网页会显示实际热点名；密码 `12345678`）
2. 浏览器访问：`http://192.168.4.1`
3. 使用页面控制开关、亮度、定时，数据每 2 秒自动刷新

## 网络模式（热点 + 局域网）

设备支持两种工作方式，开机时自动选择：

1. **首次使用（无 Wi-Fi 凭证）**：自动进入热点模式，建立 `ESPSwitch-XXXX` 热点（密码 `12345678`），手机/电脑连上后用浏览器访问 `http://192.168.4.1` 完成配置。
2. **已配置 Wi-Fi 后**：开机尝试连接保存的 Wi-Fi（STA 模式，超时 20 秒）。连接成功后使用局域网 IP，之后都走 LAN；若连接失败则自动回退到热点，仍可访问。
3. **局域网访问地址**：连接成功后页面和串口会显示局域网 IP；也可直接用主机名 `http://espswitch-XXXX.local`（mDNS）访问，配网成功后网页会自动跳转到该地址。
...
> 提示：在局域网模式下，若忘记 IP，可通过串口监视器（115200）查看，或使用 `espswitch-XXXX.local` 主机名访问（Windows 上 `.local` 可能解析不了，用 IP 访问）。
...
- 电流检测放大器 **INA180A1**，固定增益 `Gain = 20`
- 高侧分流电阻 + 电阻分压后送入 ADC，**ESP32 系与 ESP8285 系参数不同**（见下表）
...
设备连上局域网（STA 模式）后通过 **NTP 同步真实时间**（UTC+8），定时按墙钟时刻精确触发：

- 开机后会阻塞等待 NTP 同步（最多约 3 秒），保证灯状态与定时对齐
- 设置 07:00 开、23:00 关，设备就在每天 07:00 开灯、23:00 关灯
- 若设置跨夜（例如 23:00 开、07:00 关），程序会自动处理跨天逻辑
- **未联网/未同步时**（如 AP 热点模式）回落到"开机后分钟数"粗略计时，联网后自动切换回真实时间
...
**Q：网页打不开**
A：确认手机已连接 `ESPSwitch-XXXX` 热点，并输入 `http://192.168.4.1`（不是 https）。
